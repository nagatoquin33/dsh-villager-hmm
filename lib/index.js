...
/**
 * Absolute path of the asset fetcher, shown in the panel.
 *
 * Not `npx dsh-villager-hmm-assets`: the bin name differs from the package
 * name, so npx only resolves it from a directory where the package's bin is
 * linked (the profile directory). From anywhere else npx tries to fetch a
 * package literally called `dsh-villager-hmm-assets`, which does not exist.
 * An absolute path works from any working directory.
 */
export const FETCH_SCRIPT = fileURLToPath(new URL('../scripts/fetch-assets.mjs', import.meta.url))
...
/**
 * Asset file names inside the cache's `assets/` directory. The order of
 * `SOUND_FILES` and `HURT_FILES` defines the `/sound/<index>.ogg` and
 * `/hurt/<index>.ogg` routes.
 *
 * `TEXTURE_FILE` alone is NOT a dressed villager. The robe is the model's second
 * body cube (uv [0,38], 8x18x6, inflate 0.5): its pixels are fully transparent
 * in the base skin, because the robe colour lives in the villager TYPE overlay
 * that the game draws on top with the same UVs. Without `TYPE_FILE` the figure
 * still renders, but it wears only the base skin's under-robe.
 */
export const SOUND_FILES = ['idle1.ogg', 'idle2.ogg']
export const TEXTURE_FILE = 'villager.png'
export const TYPE_FILE = 'villager-type.png'

/**
 * The damage-indicator sprite, `textures/particle/damage.png`: a single 8x8
 * dark-red heart — the only red-heart particle in the vanilla assets, and the
 * one the game draws for damage. Its interior is neutral grey on purpose, as a
 * luminance mask for the engine to tint, which is why the panel paints it
 * through the same red matrix it uses for the hurt flash.
 */
export const PARTICLE_FILE = 'damage.png'

/** `/x.png` -> cache file name, so the routes and the assets cannot drift. */
const PNG_ROUTES = { texture: TEXTURE_FILE, type: TYPE_FILE, particle: PARTICLE_FILE }

/**
 * The villager's DAMAGE sounds, played by the panel's pet interaction.
 *
 * These are named `hit*`, not `hurt*`: the game's own `sounds.json` maps the
 * `entity.villager.hurt` event onto `mob/villager/hit1..hit4`, and no
 * `hurt*.ogg` exists in the vanilla assets.
 */
export const HURT_FILES = ['hit1.ogg', 'hit2.ogg', 'hit3.ogg', 'hit4.ogg']
...
export function apply(ctx, config = {}) {
  ...
  ctx.inject(['webServer'], (host) => {
    ...
    const assetStatus = () => {
      ...
      const hurts = HURT_FILES.filter((file) => readAsset(assetDir, file) !== null)
      const texture = readAsset(assetDir, TEXTURE_FILE)
      const type = readAsset(assetDir, TYPE_FILE)
      const particle = readAsset(assetDir, PARTICLE_FILE)
      return {
        sounds,
        hurts,
    ...
    const snapshot = (since) => {
      ...
      return {
        ...
        hurtCount: status.hurts.length,
        hasTexture: status.hasTexture,
        hasType: status.hasType,
        hasParticle: status.hasParticle,
        assetDir,
        setupCommand: 'node "' + FETCH_SCRIPT + '"',
    ...
    const handle = (req, res) => {
      ...
      // The PNG assets all resolve through one table; the client asks for them
      // by route name, and an unknown name simply does not match.
      const pngMatch = /^\/(texture|type|particle)\.png$/.exec(route)
      if (pngMatch !== null) {
        const file = PNG_ROUTES[pngMatch[1]]
        const bytes = readAsset(assetDir, file)
        if (bytes === null) {
          res.writeHead(404, { 'content-type': 'text/plain; charset=utf-8' })
          res.end('dsh-villager-hmm: ' + file + ' not fetched')
          return
        }
        return sendBytes(res, bytes, 'image/png', 86400)
      }

      // Ambient "hmm" and damage clips share one shape: an index into a fixed
      // list, so an out-of-range index is a 404 rather than a path traversal.
      const audioMatch = /^\/(sound|hurt)\/(\d+)\.ogg$/.exec(route)
      if (audioMatch !== null) {
        const list = audioMatch[1] === 'hurt' ? HURT_FILES : SOUND_FILES
        const file = list[Number(audioMatch[2])]
        const bytes = file === undefined ? null : readAsset(assetDir, file)
        if (bytes === null) {
          res.writeHead(404, { 'content-type': 'text/plain; charset=utf-8' })
          res.end('dsh-villager-hmm: ' + audioMatch[1] + ' not fetched')
          return
        }
    ...
    try {
      host.effect(
        () => host.webServer.register({
          kind: 'prefix',
          path: PREFIX,
          // A malformed request must never take the harness down with it.
          handler: (req, res) => {
            try {
              handle(req, res)
            } catch (error) {
              try {
                ...
                res.end('dsh-villager-hmm: ' + String((error && error.message) || error))
              } catch {
                // The response is already gone; there is nothing left to do.
              }
            }
          },
        }),
      ...
      // A route collision or a changed webServer API must not abort the boot.
      ctx.logger?.warn?.(
        'dsh-villager-hmm: could not register %s: %s',
        PREFIX,
