 ...
 * counters over the plugin's own HTTP routes and plays a vanilla Minecraft
 * villager sound once per hit.
 *
 * Host and client halves talk over plain HTTP rather than a Remote service:
 * the payload is a counter plus a handful of small audio files, so a route
 * prefix keeps the package self-contained and needs no codegen.
 *
...
/** Asset files shipped inside the package. */
const SOUND_FILES = ['idle1.ogg', 'idle2.ogg']

/** Read one bundled asset as bytes, resolved relative to this module. */
 ...
 * @param config - optional row config (`enabled`, `mode`, `pattern`).
 */
export function apply(ctx, config = {}) {
  ...
  ctx.inject(['webServer'], (host) => {
    const sounds = SOUND_FILES.map((file) => loadAsset(file))
    let face = null
    try {
      face = loadAsset('villager-face.png')
    } catch {
      face = null
    }
    ...
    const handle = (req, res) => {
      ...
      if (route === '/face.png' && face !== null) {
        return sendBytes(res, face, 'image/png', 86400)
      }
      ...
      if (soundMatch !== null) {
        ...
        return sendBytes(res, sounds[index], 'audio/ogg', 86400)
      }
    ...
    ctx.logger?.info?.('dsh-villager-hmm: serving %s (%d sounds)', PREFIX, sounds.length)
  })
}
