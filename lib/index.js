...
export function apply(ctx, config = {}) {
  ...
  ctx.inject(['webServer'], (host) => {
    ...
    const assetStatus = () => {
      ...
      return { sounds, hurts, hasTexture: texture !== null, hasType: type !== null }
    }
    ...
    const handle = (req, res) => {
      ...
      // Two PNG assets with identical semantics: the base skin and the villager
      // type overlay the client paints on top with the same UVs.
      const pngMatch = /^\/(texture|type)\.png$/.exec(route)
      if (pngMatch !== null) {
        const file = pngMatch[1] === 'type' ? TYPE_FILE : TEXTURE_FILE
